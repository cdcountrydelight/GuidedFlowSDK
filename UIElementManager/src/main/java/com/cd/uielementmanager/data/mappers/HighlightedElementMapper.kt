 ...
 */
internal class HighlightedElementMapper :
    IBaseMapper<HighlightedElementEntity?, HighlightedElementContent?> {

    override fun mapData(data: HighlightedElementEntity?): HighlightedElementContent? {
        return if (data?.elementId == null) {
            null
        } else {
            HighlightedElementContent(
